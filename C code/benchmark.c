...
int default_sizes[] =
{ 550,900,650,800,500,1000,950,700,400,600,750,850,450 }; 
//{ 3000 };  
...
int main(int argc, char **argv)
{
	...
	char * data_s = malloc(10000 * sizeof(char));
	strcpy(data_s,"[");
	...
	for(int i_sample = 0; i_sample<nsamples; i_sample++) {
		...
		for (int isize = 0; isize < nsizes; ++isize) {
			...
			double Gflops_s, seconds = -1.0, seconds1 = -1.0;
			...
			//sprintf(data_s + strlen(data_s),"%8g",Mflops_s[isize]);
			sprintf(data_s + strlen(data_s),"%8g",seconds1);

        	if(isize < nsizes-1){
				strcat(data_s,",");
			}
        	else{
				if(i_sample < nsamples-1){    
					strcat(data_s,"],");  
					//strcat(data_s,","); 
				}else{    
					strcat(data_s,"]");
